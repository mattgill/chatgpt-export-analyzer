...
export function ReportPage() {
  const navigate = useNavigate(); const session = useExportSession(); const [snapshot, setSnapshot] = useState<AnalysisSnapshot | null>(null); const [status, setStatus] = useState<'loading' | 'empty' | 'error' | 'ready'>('loading'); const [downloading, setDownloading] = useState(false); const [downloadError, setDownloadError] = useState<string | null>(null); const worker = useRef<Worker | null>(null)
  ...
  const clear = async () => { worker.current?.terminate(); worker.current = null; session.clearSource(); await reportRepository.clear(); navigate('/') }
  const download = () => {
    if (!session.sourceFile || downloading) return
    const next = new Worker(new URL('../worker/analyze.worker.ts', import.meta.url), { type: 'module' }); worker.current = next; setDownloading(true); setDownloadError(null)
    next.onmessage = (event: MessageEvent<WorkerEvent>) => {
      const message = event.data
      if (message.type === 'error') { next.terminate(); worker.current = null; setDownloading(false); setDownloadError(message.message) }
      if (message.type === 'markdownComplete') {
        const url = URL.createObjectURL(message.archive); const anchor = document.createElement('a'); anchor.href = url; anchor.download = 'chatgpt-conversations-markdown.zip'; document.body.append(anchor); anchor.click(); anchor.remove(); window.setTimeout(() => URL.revokeObjectURL(url), 0)
        next.terminate(); worker.current = null; setDownloading(false)
      }
    }
    next.onerror = () => { next.terminate(); worker.current = null; setDownloading(false); setDownloadError('Markdown download could not be completed.') }
    next.postMessage({ type: 'exportMarkdown', file: session.sourceFile })
  }
  ...
  return <main className="page report-page"><header className="report-header"><div><p className="eyebrow">Local report</p><h1>ChatGPT export analytics</h1><p>Derived locally from {snapshot.source.name}. Conversation text is not retained.</p></div><div><Link to="/">Analyze another export</Link>{session.sourceFile ? <button type="button" onClick={download} disabled={downloading}>{downloading ? 'Preparing Markdown ZIP…' : 'Download Markdown ZIP'}</button> : <span className="download-note">Re-upload the export to download Markdown.</span>}<button type="button" onClick={() => void clear()}>Clear local report</button></div></header>
