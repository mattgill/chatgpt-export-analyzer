...
export function useExportAnalysis() {
  ...
  const analyze = useCallback((file: File) => {
    ...
    next.onmessage = (event: MessageEvent<WorkerEvent>) => {
      ...
      if (message.type === 'complete') void reportRepository.replaceLatest(message.snapshot).then(() => { next.terminate(); worker.current = null; setState({ phase: null, completed: 0, error: null, active: false }); navigate('/report') }, () => { next.terminate(); worker.current = null; setState({ phase: null, completed: 0, error: 'The report could not be saved in this browser.', active: false }) })
    }
